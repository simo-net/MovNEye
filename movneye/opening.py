...
from movneye.sensor import aps, dvs, imu
from movneye.utils.add2np import gaussian_smoothing
from movneye.utils.add2cv import undistort_img_points, load_calibration, preprocess_image
from movneye.calib.paths import SENSOR_FILE, CALIB2D_CAM_FILE, CALIB3D_ROIimg_FILE
from movneye.functional.read import read_json, sensor_info, movement_info, stimulus_info, recording_info
from movneye.calib.calib3d import find_image_roi_with_margin, compute_roi_from_bounds
...
def load_img_dataset(dataset_type: str, split: str = 'train', index: int = 0, transforms: list = None,
                     verbose: bool = True, plot: bool = True):
    builder = datasets.build_dataset(dataset_type, dataset_dir=None)
    ds = datasets.load_dataset(builder, split=split)
    ...
    img = preprocess_image(datasets.retrieve_image(sample), transforms=transforms)
    label = classes[datasets.retrieve_label(sample)]
    ...
    if plot:
        ...
        plt.imshow(img, cmap=None if (len(img.shape) == 3 and img.shape[2] == 3) else 'gray')
        plt.axis('off')
        plt.show()
