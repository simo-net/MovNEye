...
from movneye.utils.add2np import azimuthal_avg
...
class SAW(object):
    ...
    def __init__(self,
                 ...
                 min_step: float = 0., max_step: float = 6., foveola: float = 60.,
        ...
        :param min_step: [arcmins] minimum size of a single FEM step
        """
        ...
        self._min_step = min_step / self._ptu_resolution
        assert self._max_step > self._min_step
        self._grid = grid
        self._lamda = lamda
    ...
    def _find_neighbours(self, center: (int, int)) -> np.ndarray:
        ...
        outer_circle = dist_from_center <= self._max_step
        inner_circle = dist_from_center < self._min_step
        return np.where(outer_circle & ~inner_circle)
    ...
    def min_step(self) -> float:
        return self._min_step * self._ptu_resolution

    @min_step.setter
    def min_step(self, min_step: float):
        self._min_step = min_step / self._ptu_resolution
        assert self._max_step > self._min_step
...
if __name__ == '__main__':
    ...
    fem_args = dict(n_burnin=10**4, min_step=3, max_step=7, foveola=60, n_traj=60)
