...
import json
import numpy as np
from visionart.calib.paths import CHESS_FILE, SCREEN_FILE, SENSOR_FILE
...
def stimulus_info(rec_file: str) -> (str, int, int, str, str):
    assert isinstance(rec_file, str) and os.path.isfile(rec_file), 'The argument must be an existing json file.'
    info_struct = read_json(rec_file)
    ...
    return img_file, img_index, img_label, img_split, img_dataset
...
@read_json_decorator
def stimulus_label(info: dict or str) -> int or None:
    try:
