...
# VERY IMPORTANT NOTE:
# When running the functions in this file, the sudo password should be required only at the time this file is imported and
# will no longer be required later on. To do so, you must edit the sudoers file in order to extend the inactivity timeout
# (i.e. the time period in which the sudo password will be remembered in the session and after which Linux will prompt
# for your password again, which is usually around 5 minutes):
...
# during the session, i.e. after it was entered the first time). Finally, save changes and close (press CTRL + X, then press y and ENTER).
...
def inactivate_usb_timeout(usb_id: str,
    ...
    Args:
        usb_id (str, required): the ID of a USB-connected device, with {bus}-{port+level} format (e.g. you can take this
                                info using the function in visionart.usb.idfinder).
