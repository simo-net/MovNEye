...
from visionart.sensor import aps
from visionart.recording import RecordImage
from visionart.functional.store import store_sensor_info, store_image_info
from visionart.functional.read import sensor_info, monitor_info, chessboard_info, recording_info, monitor_resolution, chessboard_file
from visionart.utils.add2np import normalize
from visionart.utils.add2cv import *
...
def find_image_roi(recorded_frames_file: str = None, config_file: str = None,
    ...
    if recorded_frames_file is None:
        ...
        if store_results:
            img_info = store_image_info(img_resolution, border_img, img_strinfo)
...
def find_image_roi_with_margin(margin: float, recorded_frames_file: str = None,
    ...
    fov = np.load(CALIB3D_FOVimg_FILE)
    mon_imgshape_withmargin = (int(2 * np.abs(np.tan((fov[0] + margin) * np.pi / 180 / 2)) * tvecs[2]),
                               int(2 * np.abs(np.tan((fov[1] + margin) * np.pi / 180 / 2)) * tvecs[2]))
...
def continuous_image_roi_search(config_file: str,
    ...
    print(f'   - the image ROI is {roi_img[1][0] - roi_img[0][0]} x {roi_img[1][1] - roi_img[0][1]} and FOV is {round(fov_camimg[0])}° x {round(fov_camimg[1])}°')
    ...
    myexp = RecordImage(
        ...
        img=np.zeros(img_resolution[::-1], dtype=np.uint8), border_img=border_img, border_img_color=(255, 255, 255),
        verbose=False
    )
