...
from visionart.calib.paths import CALIB2D_CAM_FILE
from visionart.utils.add2os import listdir_flatten, keep_list_extension
from visionart.functional.read import recording_info, stimulus_info
from visionart.opening import load_roi, load_roi_with_margin
from visionart.packaging import return_preprocessed_events, find_fem_period
from visionart.filtering import return_filtered_events
from visionart.functional.wrap import write_bin


# Usage:
# >>    python3 5-preprocess-filter-store.py  -i ./data/rec  -o /home/alien/data/events/FN-MNIST
#                                             --filter_file ./sensor/configs/dvsnoisefilter_biases.json
#                                             --cut_fem  --crop_roi  --fem_margin 1
...
def main(in_dir: str, out_dir: str, filter_file: str,
    ...
    for jsonfile2load in tqdm(all_files, total=len(all_files), desc='Progress', file=sys.stdout):
        ...
        _, img_index, img_label, img_split, _ = stimulus_info(jsonfile2load)
        ...
        assert img_label is not None and isinstance(img_label, int)
        assert img_index is not None and isinstance(img_index, int)

        # Define output full path
        out_path = os.path.join(out_dir, img_split, str(img_label))
