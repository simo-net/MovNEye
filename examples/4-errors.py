...
from visionart.functional.diagnose import detect_all_problems
from visionart.opening import load_roi, load_roi_with_margin, print_imu
from visionart.functional.read import monitor_resolution, recording_info, movement_info, stimulus_info, stimulus_dataset
...
# Usage:
# >>    python3 4-errors.py  --rec_dir ./data/rec/batch_2
#                            --rec_again  --max_iters 2
...
#                            --max_hole_duration 6  --min_fem_delay 250
#
# >>    python3 4-errors.py  --rec_dir ./data/rec/batch_2
#                            --max_hole_duration 6  --min_fem_delay 250
...
def parse_args():
    ...
    # parser.add_argument('--detect_issues', action="store_true", default=False,
    #                     help='Whether to detect all files with recording issues such as acquisition-timeout or '
    #                          'process-order problems.\nDefault is False.')
    # parser.add_argument('--detect_nodata', action="store_true", default=False,
    #                     help='Whether to detect all files with missing IMU or DVS data. Default is False.')
    # parser.add_argument('--detect_holes', action="store_true", default=False,
    #                     help='Whether to detect all files with holes in the recorded events. Default is False.')
    # parser.add_argument('--detect_delay', action="store_true", default=False,
    #                     help='Whether to detect all files with too low FEM delay. Default is False.')
...
def record_again(issued_recs,
    ...
    for rec_file in issued_recs:
        ...
        imgfile, index, label, split, _ = stimulus_info(rec_file)
        if split == 'train':
        ...
        assert label == datasets.retrieve_label(sample),\
