...
from movneye.functional.read import monitor_resolution
from movneye.functional.alert import sound_alert, email_alert
...
def parse_args():
    ...
    parser.add_argument('--img_transforms', nargs='+', default=[],
                        help='The type of preprocessing functions (as a list of strings) to apply to the image.\n'
                             'Give a look at movneye.utils.add2cv.preprocess_image() to check for the possibilities.'
                             '\nDefault is an empty list (no transformations are applied to the image stimuli).')
...
def record_data(dataset_type, batch_portion, batch_start, num_batches,                         # Dataset parameters
                ...
                fem_dir, fem_delay, fem_seed, speed_bounds, speed, acceleration, step,         # Movement parameters
    ...
    assert 0 < num_batches <= tot_num_batches - batch_start, \
        ...
        f'({tot_num_batches}),\n given that we start from batch {batch_start} with each batch being ' \
    ...
    pauses4batch = images4batch // num_consecutive_recs * PAUSE_PERIOD/60
    print(f'\nThe whole session of recordings will last approximately '
          f'{round(num_batches*(duration4batch + pauses4batch + wait_time/60*(1-1/num_batches)), 1)} hours:\n'
          f'i.e. ~{int(img_duration * 1e-3)} seconds for each one of the {images4batch*num_batches} images to record\n'
          f'and ~{num_batches*images4batch//num_consecutive_recs} cooling pauses lasting {PAUSE_PERIOD} minutes each.\n'
    ...
    for batch_id in range(batch_start, batch_start+num_batches):
        ...
        myexp = RecordImageFEM(
            ...
            img_dataset=None, img_split=None, img_transforms=img_transforms,
        ...
        for split in ['train', 'test']:
            ...
            for n_img in range(batch_size[split]):
                ...
                label = classes[datasets.retrieve_label_from_batch(ds_batch[split], n_img)]
                ...
                if k_rec > num_consecutive_recs:
                    ...
                    if temperature is None:
                        ...
                        if not deact:
                            raise Exception(f'No USB found with ID {usb_id}. Aborting.')
        ...
        # Before starting next recordings' session: close device and disable its USB for a given amount of time
        myexp.close()
